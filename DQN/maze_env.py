...
class Maze(tk.Tk):
    ...
    def reset(self):
        ...
        return (np.array(self.canvas.coords(self.rect)[:2]) -
                np.array(self.canvas.coords(self.oval)[:2]))/(Height_Maze*Grid_Num)
    ...
    def step(self, action):
        ...
        s_next = (np.array(next_coords[:2]) - np.array(self.canvas.coords(self.oval)[:2]))/(Height_Maze*Grid_Num)

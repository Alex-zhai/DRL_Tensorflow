...
class DQNPrioritizedReplay:
    ...
    # 构建值网络模型，这里构建的是简单的两层的mlp网络，其中n_l1是第一隐藏层的节点数
    # 可以构建复杂的卷积神经网络来表示值函数
    def _build_net(self):
    ...
    def store_transition(self, s, a, r, s_):
        if self.prioritized:
            ...
            max_p = np.max(
                self.memory.tree.tree[-self.memory.tree.capacity:])  # have high priority for newly arrived transition
